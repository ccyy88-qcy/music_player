...
class _HomeScreenState extends State<HomeScreen>
  ...
  Future<void> _scanMusic({bool forceFull = false}) async {
    ...
    if (!granted) {
      setState(() { _loading = false; _error = '需要存储权限才能扫描音乐文件'; });
      return;
    }
    try {
      ...
      setState(() { _loading = false; _error = '扫描失败: $e'; });
    }
  }
  ...
  void _applyFilter() {
    ...
    setState(() {
      if (q.isEmpty) {
        ...
        _filteredSongs = {
          for (final e in _allSongs.entries)
            e.key: e.value.where((s) => s.title.toLowerCase().contains(q)).toList(),
        };
      }
    });
  }

  void _playList(List<Song> list, {int startIndex = 0}) {
    if (list.isEmpty) return;
    audioHandler.loadSongList(list, startIndex: startIndex);
  }
  ...
  Widget build(BuildContext context) {
    final total = (_allSongs[MusicCategory.dj]?.length ?? 0) +
    ...
    return Scaffold(
      body: Column(children: [
        ...
        if (!_showSearch)
          Container(
            ...
            child: TabBar(
              ...
              indicatorColor: Colors.pinkAccent, indicatorWeight: 3,
        ...
        Expanded(child: _loading ? _loadingView() : _error != null ? _errorView() : _showSearch ? _searchResults() : TabBarView(
          controller: _tabController,
          children: [
        ...
        if (!_showSearch) MiniPlayer(onTap: _openPlayer),
      ]),
    );
  }
  ...
  Widget _loadingView() => const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [CircularProgressIndicator(color: Colors.pinkAccent), SizedBox(height: 16), Text('扫描中...', style: TextStyle(color: Colors.grey))]));
  ...
  Widget _songList(MusicCategory cat) {
    ...
    return Column(children: [
      ...
      Expanded(child: ListView.builder(itemCount: songs.length, itemBuilder: (_, i) {
        final s = songs[i];
  ...
  bool _isPlaying(Song s) {
    final cs = audioHandler.currentSong;
    return cs != null && cs.filePath == s.filePath;
  }

  Future<void> _toggleFav(Song s) async {
    if (_store == null) return;
    await _store!.toggleFavorite(s.id);
    setState(() {});
  }
}
